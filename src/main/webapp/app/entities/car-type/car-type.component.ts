...
export class CarTypeComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.carTypeService
      ...
      .subscribe((res: HttpResponse<ICarType[]>) => this.paginateCarTypes(res.body, res.headers));
  }
  ...
  trackId(index: number, item: ICarType) {
    return item.id;
  }

  registerChangeInCarTypes() {
    this.eventSubscriber = this.eventManager.subscribe('carTypeListModification', () => this.loadAll());
  }

  delete(carType: ICarType) {
    const modalRef = this.modalService.open(CarTypeDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected paginateCarTypes(data: ICarType[], headers: HttpHeaders) {
